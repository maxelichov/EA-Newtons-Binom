...
public class LogicManager
{
    ...
    public void filterCourses(Preferences pref)
    {
        ...
        {
            Course newCourse = new Course(currCourse.getCourseName(),currCourse.getCredits(),currCourse.getDifficulty(),currCourse.getTestA(),currCourse.getTestB(),currCourse.getIsMandatory());
            ...
            for(Lesson practice: currCourse.getPractices()) //todo: fix coursePractice is null
            {

                if (pref.getUnavailablityWeekSchedule().isAvailabile(practice.getDay(), practice.getTimeRange()))
                {
            ...
            {
                if(currCourse.getPractices().size()==0){
                    res.add(newCourse);
                }else{
                    if(newCourse.getPractices().size() > 0){
                        res.add(newCourse);
                    }
                }
            }
    ...
    public Group isAllLessonAvailableInAGroup(Group group, WeekSchedule unAvailableAt)
    {
        ...
        {
            if (unAvailableAt.isAvailabile(currLesson.getDay(), currLesson.getTimeRange()))
            {
    ...
    public Schedule startEngine() throws Exception
    {
