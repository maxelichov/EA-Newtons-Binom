...
public class ScheduleFactory implements CandidateFactory {
    ...
    private Course cloneCourseWithOnlyOneGroup(Course course) {
        ...
        try {
            ...
            if(course.getPractices().size()!=0){
                int randIndexForPractice = random.nextInt(course.getPractices().size());
                newCoursePractices = Arrays.asList(newCourse.getPractices().get(randIndexForPractice));
            }
